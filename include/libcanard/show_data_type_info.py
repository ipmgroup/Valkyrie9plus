...
try:
    ...
    sys.stderr.write(
        "pyDroneCAN is not installed. Please install from PIP: sudo pip3 install dronecan\n"
    )
    exit(1)
...
if __name__ == "__main__":
    ...
    header = (
        "Full Data Type Name".ljust(longest_name)
        + " | DDTID |   Type Signature   |  Max Bit Len  "
    )
    print(header)
    print("-" * len(header))
    ...
    for typename, typedef in sorted(dronecan.TYPENAMES.items()):
        ...
        s = "%-*s   % 5s   0x%016x" % (
            ...
            typename,
            ddtid,
            typedef.get_data_type_signature(),
        )
        try:
            s += "   % 5d" % typedef.get_max_bitlen()
        except Exception:
            s += "   % 5d / %-5d" % (
                typedef.get_max_bitlen_request(),
                typedef.get_max_bitlen_response(),
            )
        print(s)
