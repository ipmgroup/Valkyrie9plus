...
#if (CANARD_STM32_CAN_TIR_RTR != CANARD_STM32_CAN_RIR_RTR) || (CANARD_STM32_CAN_TIR_IDE != CANARD_STM32_CAN_RIR_IDE)
...
int16_t canardSTM32Init(const CanardSTM32CANTimings* const timings,
     ...
     * Paranoia time.
     */
    if ((iface_mode != CanardSTM32IfaceModeNormal) && (iface_mode != CanardSTM32IfaceModeSilent) && (iface_mode != CanardSTM32IfaceModeAutomaticTxAbortOnError)) {
        return -CANARD_ERROR_INVALID_ARGUMENT;
    }

    if ((timings == NULL) || (timings->bit_rate_prescaler < 1) || (timings->bit_rate_prescaler > 1024) || (timings->max_resynchronization_jump_width < 1) || (timings->max_resynchronization_jump_width > 4) || (timings->bit_segment_1 < 1) || (timings->bit_segment_1 > 16) || (timings->bit_segment_2 < 1) || (timings->bit_segment_2 > 8)) {
        return -CANARD_ERROR_INVALID_ARGUMENT;
    }
    ...
    BXCAN->BTR = (((timings->max_resynchronization_jump_width - 1U) & 3U) << 24U) | (((timings->bit_segment_1 - 1U) & 15U) << 16U) | (((timings->bit_segment_2 - 1U) & 7U) << 20U) | ((timings->bit_rate_prescaler - 1U) & 1023U) | ((iface_mode == CanardSTM32IfaceModeSilent) ? CANARD_STM32_CAN_BTR_SILM : 0);
...
int16_t canardSTM32Transmit(const CanardCANFrame* const frame) {
    ...
    mb->TDHR = (((uint32_t)frame->data[7]) << 24U) | (((uint32_t)frame->data[6]) << 16U) | (((uint32_t)frame->data[5]) << 8U) | (((uint32_t)frame->data[4]) << 0U);
    mb->TDLR = (((uint32_t)frame->data[3]) << 24U) | (((uint32_t)frame->data[2]) << 16U) | (((uint32_t)frame->data[1]) << 8U) | (((uint32_t)frame->data[0]) << 0U);
...
int16_t canardSTM32ConfigureAcceptanceFilters(const CanardSTM32AcceptanceFilterConfiguration* const filter_configs,
    ...
    if ((filter_configs == NULL) || (num_filter_configs > CANARD_STM32_NUM_ACCEPTANCE_FILTERS)) {
        return -CANARD_ERROR_INVALID_ARGUMENT;
    }
