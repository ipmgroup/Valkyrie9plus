...
bool _uavcan_protocol_param_GetSetResponse_decode(const CanardRxTransfer* transfer, uint32_t* bit_ofs, struct uavcan_protocol_param_GetSetResponse* msg, bool tao) {
    ...
    *bit_ofs += 5;
    ...
    if (_uavcan_protocol_param_Value_decode(transfer, bit_ofs, &msg->value, false)) {
        return true;
    }

    *bit_ofs += 5;

    if (_uavcan_protocol_param_Value_decode(transfer, bit_ofs, &msg->default_value, false)) {
        return true;
    }

    *bit_ofs += 6;

    if (_uavcan_protocol_param_NumericValue_decode(transfer, bit_ofs, &msg->max_value, false)) {
        return true;
    }

    *bit_ofs += 6;

    if (_uavcan_protocol_param_NumericValue_decode(transfer, bit_ofs, &msg->min_value, false)) {
        return true;
    }
