...
struct uavcan_protocol_GetTransportStatsResponse {
    ...
    struct {
        uint8_t                              len;
        struct uavcan_protocol_CANIfaceStats data[3];
    } can_iface_stats;
};
    ...
    *bit_ofs += 48;
    ...
    if (tao) {
        ...
        while (max_bits > *bit_ofs) {
            if (!max_len-- || _uavcan_protocol_CANIfaceStats_decode(transfer, bit_ofs, &msg->can_iface_stats.data[msg->can_iface_stats.len], false)) {
                return true;
            }
...
#pragma GCC diagnostic pop
        ...
        for (size_t i = 0; i < msg->can_iface_stats.len; i++) {
            if (_uavcan_protocol_CANIfaceStats_decode(transfer, bit_ofs, &msg->can_iface_stats.data[i], false)) {
                return true;
            }
        }
    }
