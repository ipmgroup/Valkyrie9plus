...
struct uavcan_equipment_ice_reciprocating_Status {
    ...
    struct {
        uint8_t                                                  len;
        struct uavcan_equipment_ice_reciprocating_CylinderStatus data[16];
    } cylinder_status;
};
    ...
    *bit_ofs += 3;
    ...
    if (tao) {
        ...
        while (max_bits > *bit_ofs) {
            if (!max_len-- || _uavcan_equipment_ice_reciprocating_CylinderStatus_decode(transfer, bit_ofs, &msg->cylinder_status.data[msg->cylinder_status.len], false)) {
                return true;
            }
...
#pragma GCC diagnostic pop
        ...
        for (size_t i = 0; i < msg->cylinder_status.len; i++) {
            if (_uavcan_equipment_ice_reciprocating_CylinderStatus_decode(transfer, bit_ofs, &msg->cylinder_status.data[i], false)) {
                return true;
            }
        }
    }
