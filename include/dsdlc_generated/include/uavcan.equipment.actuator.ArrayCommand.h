...
struct uavcan_equipment_actuator_ArrayCommand {
    ...
    struct {
        uint8_t                                  len;
        struct uavcan_equipment_actuator_Command data[15];
    } commands;
};
...
bool _uavcan_equipment_actuator_ArrayCommand_decode(const CanardRxTransfer* transfer, uint32_t* bit_ofs, struct uavcan_equipment_actuator_ArrayCommand* msg, bool tao) {
    ...
    if (tao) {
        ...
        while (max_bits > *bit_ofs) {
            if (!max_len-- || _uavcan_equipment_actuator_Command_decode(transfer, bit_ofs, &msg->commands.data[msg->commands.len], false)) {
                return true;
            }
            msg->commands.len++;
        }
...
#pragma GCC diagnostic pop
        ...
        for (size_t i = 0; i < msg->commands.len; i++) {
            if (_uavcan_equipment_actuator_Command_decode(transfer, bit_ofs, &msg->commands.data[i], false)) {
                return true;
            }
        }
    }
