...
bool _uavcan_protocol_GetNodeInfoResponse_decode(const CanardRxTransfer* transfer, uint32_t* bit_ofs, struct uavcan_protocol_GetNodeInfoResponse* msg, bool tao) {
    ...
    (void)msg;
    (void)tao;
    if (_uavcan_protocol_NodeStatus_decode(transfer, bit_ofs, &msg->status, false)) {
        return true;
    }

    if (_uavcan_protocol_SoftwareVersion_decode(transfer, bit_ofs, &msg->software_version, false)) {
        return true;
    }

    if (_uavcan_protocol_HardwareVersion_decode(transfer, bit_ofs, &msg->hardware_version, false)) {
        return true;
    }
