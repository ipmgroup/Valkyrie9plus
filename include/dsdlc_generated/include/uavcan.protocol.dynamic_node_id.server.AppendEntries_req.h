...
struct uavcan_protocol_dynamic_node_id_server_AppendEntriesRequest {
    ...
    struct {
        uint8_t                                             len;
        struct uavcan_protocol_dynamic_node_id_server_Entry data[1];
    } entries;
};
    ...
    *bit_ofs += 8;
    ...
    if (tao) {
        ...
        while (max_bits > *bit_ofs) {
            if (!max_len-- || _uavcan_protocol_dynamic_node_id_server_Entry_decode(transfer, bit_ofs, &msg->entries.data[msg->entries.len], false)) {
                return true;
            }
            msg->entries.len++;
        }
...
#pragma GCC diagnostic pop
        ...
        for (size_t i = 0; i < msg->entries.len; i++) {
            if (_uavcan_protocol_dynamic_node_id_server_Entry_decode(transfer, bit_ofs, &msg->entries.data[i], false)) {
                return true;
            }
        }
    }
