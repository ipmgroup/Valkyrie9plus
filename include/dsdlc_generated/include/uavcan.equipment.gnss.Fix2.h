...
struct uavcan_equipment_gnss_Fix2 {
    ...
    struct {
        uint8_t                                           len;
        struct uavcan_equipment_gnss_ECEFPositionVelocity data[1];
...
bool _uavcan_equipment_gnss_Fix2_decode(const CanardRxTransfer* transfer, uint32_t* bit_ofs, struct uavcan_equipment_gnss_Fix2* msg, bool tao) {
    ...
    (void)msg;
    (void)tao;
    if (_uavcan_Timestamp_decode(transfer, bit_ofs, &msg->timestamp, false)) {
        return true;
    }

    if (_uavcan_Timestamp_decode(transfer, bit_ofs, &msg->gnss_timestamp, false)) {
        return true;
    }
    ...
    *bit_ofs += 16;
    ...
    if (tao) {
        ...
        while (max_bits > *bit_ofs) {
            if (!max_len-- || _uavcan_equipment_gnss_ECEFPositionVelocity_decode(transfer, bit_ofs, &msg->ecef_position_velocity.data[msg->ecef_position_velocity.len], false)) {
                return true;
            }
...
#pragma GCC diagnostic pop
        ...
        for (size_t i = 0; i < msg->ecef_position_velocity.len; i++) {
            if (_uavcan_equipment_gnss_ECEFPositionVelocity_decode(transfer, bit_ofs, &msg->ecef_position_velocity.data[i], false)) {
                return true;
            }
        }
    }
