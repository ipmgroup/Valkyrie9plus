...
//             break;
//         case Bitrate_500K:
//             ECanaShadow.CANBTC.all = 0x00050059;
//             break;
//         case Bitrate_250K:
//             ECanaShadow.CANBTC.all = 0x000b0059;
//             break;
//         case Bitrate_125K:
//             ECanaShadow.CANBTC.all = 0x00170059;
//             break;
//         case Bitrate_100K:
//             ECanaShadow.CANBTC.all = 0x001d0059;
//             break;
//         case Bitrate_83K:
//             ECanaShadow.CANBTC.all = 0x00230059;
//             break;
//         case Bitrate_50K:
//             ECanaShadow.CANBTC.all = 0x003b0059;
//             break;
//         case Bitrate_20K:
//             ECanaShadow.CANBTC.all = 0x00950059;
//             break;
//         default:
//             ECanaShadow.CANBTC.all = 0x000503BD;
//             break;  // 500K 100ns  sprac35
...
void ECAN_setBitrate(ECAN_Handle handle, uint32_t bitrate) {
    struct ECAN_REGS ECanaShadow;
...
void ECAN_configMailbox(ECAN_Handle handle, ECAN_MailBox_e MailBoxN, uint32_t msgid, Enable_Mbox_e enable_t, ECAN_MailDir_e dir_t, IDE_Bit_e IDE_t, DLC_Bit_e length, OPC_Bit_e opc_t, LAMI_Bit_e lami_bit, AME_Bit_e AME_t, uint32_t mask) {
    ...
    if (IDE_t == Extended_ID) {
        mbox->MSGID.all &= 0xE0000000;
        mbox->MSGID.all |= msgid & 0x1FFFFFFF;
    } else {
...
bool ECAN_sendExtendedFrameToMbox(ECAN_Handle handle, ECAN_MailBox_e MailBoxN, MSG_t *canFrame, uint16_t remoteTransfer, uint16_t priority) {
    if ((handle->ECanaRegs->CANMD.all & (1 << MailBoxN)) && ((remoteTransfer & 1) == 0)) {
        return false;
    }

    if (handle->ECanaRegs->CANTRS.all & (1 << MailBoxN)) {
        return false;
    }

    ECAN_MailBox_Enable(handle, MailBoxN, Disable_Mbox);
    // handle->ECanaRegs->CANME.all &= ~(1 << MailBoxN);

    volatile struct MBOX *mbox = (&(handle->ECanaMboxes->MBOX0)) + MailBoxN;

    mbox->MDL.all         = canFrame->dataL;
    mbox->MDH.all         = canFrame->dataH;
    mbox->MSGCTRL.bit.DLC = canFrame->dataLength;
    mbox->MSGCTRL.bit.RTR = remoteTransfer & 1;
    mbox->MSGCTRL.bit.TPL = priority & 0x1F;

    mbox->MSGID.all &= 0xE0000000;
    mbox->MSGID.all |= (canFrame->msgID & 0x1FFFFFFF);
    mbox->MSGID.bit.IDE = 1;

    ECAN_MailBox_Enable(handle, MailBoxN, Enable_Mbox);
    // handle->ECanaRegs->CANME.all |= (1 << MailBoxN);

    // Указываем, что кадр готов к передаче.
    handle->ECanaRegs->CANTRS.all |= (1 << (uint32_t)MailBoxN);

    return true;
}

bool ECAN_sendFrameToMbox(ECAN_Handle handle, ECAN_MailBox_e MailBoxN, MSG_t *canFrame, uint16_t remoteTransfer, uint16_t priority) {
    // Проверка: если почтовый ящик настроен для RX, а это не запрос на передачу, выход.
    if ((handle->ECanaRegs->CANMD.all & (1 << MailBoxN)) && ((remoteTransfer & 1) == 0)) {
        return false;
    }

    // Проверка: если в почтовом ящике уже есть данные для передачи, выход.
    if (handle->ECanaRegs->CANTRS.all & (1 << MailBoxN)) {
        return false;
    }
    ...
    // Указатель на почтовый ящик
    volatile struct MBOX *mbox = (&(handle->ECanaMboxes->MBOX0)) + MailBoxN;

    // Установка данных кадра
    mbox->MDL.all         = canFrame->dataL;
    mbox->MDH.all         = canFrame->dataH;
    mbox->MSGCTRL.bit.DLC = canFrame->dataLength;
    mbox->MSGCTRL.bit.RTR = remoteTransfer & 1;
    ...
    // Определение типа идентификатора и установка ID
    if (canFrame->msgID > 0x7FF) {                          // Если ID больше 11 бит, это расширенный идентификатор
        mbox->MSGID.all &= 0xE0000000;                      // Очищаем 29 младших бит
        mbox->MSGID.all |= (canFrame->msgID & 0x1FFFFFFF);  // Устанавливаем 29-битный идентификатор
        mbox->MSGID.bit.IDE = 1;                            // Устанавливаем бит IDE
    } else {
        mbox->MSGID.all &= 0xFFFFFC00;                          // Очищаем стандартный идентификатор
        mbox->MSGID.bit.STDMSGID = (canFrame->msgID & 0x03FF);  // Устанавливаем 11-битный стандартный идентификатор
        mbox->MSGID.bit.IDE      = 0;                           // Сбрасываем бит IDE
    }
...
bool ECAN_sendFrameToAvailableTxMbox(ECAN_Handle handle, MSG_t *canFrame, uint16_t remoteTransfer, uint16_t priority) {
    uint32_t availableTxMboxes = ~(handle->ECanaRegs->CANMD.all) & ~(handle->ECanaRegs->CANTRS.all);

    if (availableTxMboxes == 0) {
        return false;
    }

    for (uint16_t mbox = 0; mbox < 32; mbox++) {
        if ((availableTxMboxes >> mbox) & 1) {
            return ECAN_sendFrameToMbox(handle, (ECAN_MailBox_e)mbox, canFrame, remoteTransfer, priority);
        }
    }
...
bool ECAN_sendExtendedFrameToAvailableTxMbox(ECAN_Handle handle, MSG_t *canFrame, uint16_t remoteTransfer, uint16_t priority) {
    uint32_t availableTxMboxes = ~(handle->ECanaRegs->CANMD.all) & ~(handle->ECanaRegs->CANTRS.all);
    ...
    for (uint16_t mbox = 0; mbox < 32; mbox++) {
        if ((availableTxMboxes >> mbox) & 1) {
            if (ECAN_sendExtendedFrameToMbox(handle, (ECAN_MailBox_e)mbox, canFrame, remoteTransfer, priority)) {
                return true;
            } else {
                return false;
            }
...
bool ECAN_checkMail(ECAN_Handle handle) {
    if (handle->ECanaRegs->CANRMP.all == 0)
        return false;
    else
        return true;
}
...
bool ECAN_getMsgFIFO_ID(ECAN_Handle handle, ECAN_MailBox_e MailBoxN, FIFO_ID_Obj *pECAN_FIFO) {
    ...
    if (mbox->MSGID.bit.IDE) {
        msg.msgID = mbox->MSGID.all & 0x1FFFFFFF;
    } else {
...
//     uint32_t rmp = handle->ECanaRegs->CANRMP.all;
//     bool all_successful = true;

//     for (int16_t mbox_i = pECAN_Mailbox->RX_max; (mbox_i >= pECAN_Mailbox->RX_min) && rmp != 0; mbox_i--) {
//         if (rmp & (((uint32_t)1) << mbox_i)) {
//             bool successful = ECAN_getMsgFIFO_ID(handle, (ECAN_MailBox_e)mbox_i, pECAN_FIFO);
//             all_successful &= successful;
//             rmp = ~(~rmp | (((uint32_t)1) << mbox_i));
//         }
//     }
...
bool ECAN_getMsgFIFO_ID_N(ECAN_Handle handle, ECAN_Mailbox *pECAN_Mailbox, FIFO_ID_Obj *pECAN_FIFO) {
    ...
    for (int16_t mbox_i = pECAN_Mailbox->RX_max; (mbox_i >= pECAN_Mailbox->RX_min) && (rmp != 0); mbox_i--) {
        uint32_t mask = ((uint32_t)1 << mbox_i);
        if (rmp & mask) {
            bool successful = ECAN_getMsgFIFO_ID(handle, (ECAN_MailBox_e)mbox_i, pECAN_FIFO);
            if (!successful) {
